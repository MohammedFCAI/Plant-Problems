...
{
	public class CommentHandlerCommand : ResponseHandler, IRequestHandler<AddCommentRequestCommand, Response<Comment>>
		, IRequestHandler<UpdateCommentRequestCommand, Response<string>>, IRequestHandler<DeleteCommentRequestCommand, Response<string>>
	{
		...
		public async Task<Response<Comment>> Handle(AddCommentRequestCommand request, CancellationToken cancellationToken)
		{

			var userResponse = await _userService.GetUserById(request.UserId);
			if (!userResponse.Success)
				return BadRequest<Comment>(userResponse.Message);
			...
			if (!commentResponse.Success)
				return NotFound<Comment>(commentResponse.Message);
			var comment = commentResponse.Entities;
			return Success(comment, commentResponse.Message, 1);
		}
